...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR lpCmdLine, int nCmdShow)
{
    ...
    /* System icon: LoadIcon is rendered reliably by every capture path
       (the self-drawn CreateIconIndirect icon produced a 0x0 bitmap in
       trayhost's GetIconInfo-based capture). */
    g_icon = LoadIconW(NULL, IDI_APPLICATION);
    if (!g_icon) {
        g_icon = LoadIconW(NULL, IDI_SHIELD);
    }
