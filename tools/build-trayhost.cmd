...
rem
rem Produces:
rem   <repo>\tmp\trayhost_out\trayhost.exe
rem
...
set "INC=-I%CRAFT%\include -I%CRAFT%\include\QtCore -I%CRAFT%\include\QtDBus -I%CRAFT%\include\KF6 -I%CRAFT%\mkspecs\win32-msvc -I%SRC%"
for /d %%D in ("%CRAFT%\include\KF6\*") do set "INC=!INC! -I%%D"
set "DEFS=-DUNICODE -D_UNICODE -DWIN32 -DWIN64 -DNOMINMAX -DQT_NO_DEBUG -DQT_CORE_LIB -DQT_DBUS_LIB -DQT_GUI_LIB"
set "LIBS=/LIBPATH:%CRAFT%\lib Qt6Core.lib Qt6DBus.lib user32.lib shell32.lib gdi32.lib"

echo === moc ===
"%CRAFT%\bin\moc.exe" %INC% "%SRC%\windowstrayhost.h" -o "%OUT%\moc_windowstrayhost.cpp"
"%CRAFT%\bin\moc.exe" %INC% "%SRC%\snibridge.h" -o "%OUT%\moc_snibridge.cpp"
if errorlevel 1 exit /b 1

echo === cl ===
cl /nologo /EHsc /MD /utf-8 /O2 /Ob1 /DNDEBUG /Zc:__cplusplus /permissive- /std:c++17 %DEFS% %INC% -I"%OUT%" ^
    "%SRC%\trayhost_main.cpp" "%SRC%\windowstrayhost.cpp" "%SRC%\snibridge.cpp" "%OUT%\moc_windowstrayhost.cpp" "%OUT%\moc_snibridge.cpp" ^
